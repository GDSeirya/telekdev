function map_weater.init()
	
	local soundhax = load_particle("soundhax.obj")

	function map_weater.orbInit(object)
		local x, y = object:pos()
		object:data().xTarg = x
		object:data().yTarg = y
	end
	
	function map_weater.orbUpdateTarg(object)
		...
		if worm then
			...
			elseif vector_distance( object:data().xTarg,object:data().yTarg, object:pos() ) < 0 then
				local newx, newy = angle_vector( randomfloat(0,10), randomfloat(5,150) )
				newx = newx + oldx
				newy = newy + oldy
	...
	function map_weater.orbThink(object)
		local worm = object:get_closest_worm()
	...
	function map_weater.orbClank(object)
